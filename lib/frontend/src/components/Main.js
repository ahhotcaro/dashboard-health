...
export default function Main() {
  ...
  useEffect(() => {
    ...
    const fetchRandomSport = async () => {
      const idSport = [1, 4, 5, 960, 12, 19, 20, 23, 29, 34, 36, 37, 38, 39, 40, 44, 45, 46, 49, 51, 57, 58, 59, 66, 67, 68, 69, 70, 73, 74, 76, 78, 80, 81, 82, 83, 84, 86, 87, 89, 90, 91, 92, 93, 94, 95, 96, 98, 100, 101, 103, 105, 106, 109];
      ...
      const newRandomSport = {
        ...
        sportPic: data.data?.relationships?.images?.data[0]?.variants[0].thumbnail.url,
...
<div className="mosaique">
            ...
            >
              ...
              <p className="impData">{randomSport?.sportName && randomSport.sportName} </p>
              <p className="food"> {randomSport?.sportDescription && randomSport.sportDescription} </p>
              
              <div className="food">

                {(randomSport.sportPic || randomSport.sportIcon) && (
                  <img src={randomSport.sportPic? randomSport.sportPic:randomSport.sportIcon &&randomSport.sportIcon } width="150" height="150"/>
                )}
              </div>
            </Card>
          </div>
